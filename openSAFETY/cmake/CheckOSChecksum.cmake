...
MACRO(CheckCRCUnitTest STACK_PART LIST_FILE SOURCE_FILE CHECKER )

    IF ( CHECKSUM_SRC_UNITTEST )

        FOREACH(_currentItem ${SOURCE_FILE})
            STRING(REPLACE "${CMAKE_SOURCE_DIR}/unittest/eplssrc/" "" FILE_PATH ${_currentItem} )
            STRING(SUBSTRING ${FILE_PATH} 0 3 PATH_START)

            IF ( PATH_START STREQUAL "TST" )

                STRING(FIND ${FILE_PATH} "/" SLASH_POS)
                IF ( ${SLASH_POS} GREATER 0 )
                  STRING(SUBSTRING ${FILE_PATH} 0 ${SLASH_POS} TST_NAME)
                  STRING(REPLACE "${CMAKE_SOURCE_DIR}/unittest/eplssrc/${TST_NAME}/" "" FILE_PATH ${_currentItem} )
                  STRING(FIND ${FILE_PATH} "/" SLASH_POS)
                  STRING(SUBSTRING ${FILE_PATH} 0 ${SLASH_POS} TST_SUBDIR)

                  AddCRCCheck ( "${LIST_FILE}" "UNITTEST" "${TST_NAME}/${TST_SUBDIR}" "${_currentItem}" "${CMAKE_BINARY_DIR}/crc/unittest/eplssrc/${TST_NAME}/${TST_SUBDIR}" )
                ENDIF ()
            ENDIF ()

        ENDFOREACH(_currentItem)

        ADD_DEPENDENCIES ( CRC_CHECK_UNITTEST CRC_CHECK_UNITTESTS )

    ENDIF ( CHECKSUM_SRC_UNITTEST )
ENDMACRO(CheckCRCUnitTest)

ADD_CUSTOM_TARGET ( "CRC_CHECK_STACK" ALL
    COMMENT "Checking openSAFETY Stack crc"
)
ADD_CUSTOM_TARGET ( "CRC_CHECK_UNITTESTS" ALL
    COMMENT "Checking openSAFETY Unittest crc"
)

IF ( TARGET oscrccheck_marker )
    ADD_DEPENDENCIES ( CRC_CHECK_STACK oscrccheck oscrccheck_marker )
    ADD_DEPENDENCIES ( CRC_CHECK_UNITTESTS oscrccheck oscrccheck_marker )
ENDIF()

SET ( openSAFETY_STACK_FILE "${oscrccheck_SOURCE_DIR}/oschecksum.crc" )
SET ( openSAFETY_STACK_CRC "d48a7279" )
SET ( openSAFETY_UNITTEST_FILE "${oscrccheck_SOURCE_DIR}/osunittest.crc" )
SET ( openSAFETY_UNITTEST_CRC "a0087bac" )
...
ADD_CUSTOM_COMMAND (
    ...
    COMMAND ${CMAKE_COMMAND} -D CRC_LOCATION=${oschecksumLocation}
        ...
        -D openSAFETY_UNITTEST_CRC=${openSAFETY_UNITTEST_CRC} -D openSAFETY_UNITTEST_FILE=${openSAFETY_UNITTEST_FILE}
